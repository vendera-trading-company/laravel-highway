...
abstract class Highway
{
    ...
    private static function _method(mixed $entity, string $path, mixed $method, mixed $routeType = 'get')
    {
        ...
        $class = $namespace . '\\Controllers\\' . ucfirst($id) . 'Controller';

        if (!method_exists($class, $methodName)) {
            if ($routeType == 'get') {
                Route::$routeType($path, function () use ($methodName, $namespace, $id, $routeName, $entity, $context_name) {
                    return App::call($namespace . '\\Controllers\\' . $id . 'Controller@view', [
                        'entity' => $entity,
                        'type' => $methodName,
                        'namespace' => $namespace,
                        'routeName' => $routeName,
                ...
                Route::$routeType($path, function () use ($methodName, $namespace, $id, $routeName, $entity, $context_name) {
                    return App::call($namespace . '\\Controllers\\' . $id . 'Controller@action', [
                        'entity' => $entity,
                        'type' => $methodName,
                        'namespace' => $namespace,
                        'routeName' => $routeName,
            ...
            Route::$routeType($path, $namespace . '\\Controllers\\' . $id . 'Controller@' . $methodName)->name($routeName);
        }
    }
